...
def generate():
    ...
    if not cards:
        print("⚠️ ins_confession: статьи не найдены.")
        return

    fg = FeedGenerator()
    fg.title('The Insider — Исповедь')
    fg.link(href=URL, rel='alternate')
    fg.description('RSS-лента раздела «Исповедь» сайта The Insider')
    fg.language('ru')
    ...
    print(f"✅ ins_confession: сгенерирован ins_confession.xml ({len(seen_links)} статей)")
