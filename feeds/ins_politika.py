...
def generate():
    ...
    if not cards:
        print("⚠️ ins_politika: статьи не найдены.")
        return

    fg = FeedGenerator()
    fg.title('The Insider — Политика')
    fg.link(href=URL, rel='alternate')
    fg.description('RSS-лента раздела «Политика» сайта The Insider')
    fg.language('ru')
    ...
    print(f"✅ ins_politika: сгенерирован ins_politika.xml ({len(seen_links)} статей)")
