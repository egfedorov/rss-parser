...
def generate():
    url = 'https://ngs55.ru/authors/ekaterina-shrainer-id31731/'
    response = requests.get(url)
    response.encoding = 'utf-8'  # Явно задаём
    soup = BeautifulSoup(response.text, 'html.parser')

    fg = FeedGenerator()
    fg.title('Екатерина Шрайнер')
    fg.link(href=url, rel='alternate')
    fg.description('NGS55 – Омск')
    fg.language('ru')
    ...
    fg.rss_file('shrainer.xml', encoding='utf-8')
